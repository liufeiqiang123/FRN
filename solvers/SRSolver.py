...
class SRSolver(BaseSolver):
    def __init__(self, opt):
        ...
        if self.is_train:
            ...
            if loss_type == 'l1':
                ...
                self.criterion_pix = nn.SmoothL1Loss()
            else:
    ...
    def _net_init(self, init_type='kaiming'):
        ...
        #init_weights(self.model, init_type)  # 不使用凯明初始化
    ...
    def save_current_log(self):
        self.records['train_loss'].append(self.records['train_loss'][self.best_epoch - 1])
        self.records['val_loss'].append(self.records['val_loss'][self.best_epoch - 1])
        self.records['psnr'].append(self.records['psnr'][self.best_epoch - 1])
        self.records['ssim'].append(self.records['ssim'][self.best_epoch - 1])
        self.records['lr'].append(self.records['lr'][self.best_epoch - 1])
        ...
        self.records['train_loss'].pop()
        self.records['val_loss'].pop()
        self.records['psnr'].pop()
        self.records['ssim'].pop()
        self.records['lr'].pop()
